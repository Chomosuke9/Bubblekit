...
- **Bring-your-own model integration**: No LLM/provider SDK is bundled; `apps/server/main.py` is intentionally empty until you register handlers and install your chosen SDK. Trade-off: more setup for contributors, but avoids unwanted dependencies.
...
- **Concurrency safety**: The runtime assumes single-threaded event loops; any globals added in handler code can race if reused concurrently.
